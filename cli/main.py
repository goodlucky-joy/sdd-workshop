...
def format_todo(todo):
    ...
    return f"{todo.id:>3} {status} {todo.title} | due={due} | priority={priority}"
...
def add(
    ...
    try:
        todo = add_todo(title=title, due_date=due, priority=priority)
...
def list_items(
    ...
    try:
        todos = list_todos(done=status, priority=priority)
        if not todos:
